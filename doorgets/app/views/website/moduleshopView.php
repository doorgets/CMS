...
class moduleShopView extends doorgetsWebsiteView{
    ...
    public function getContent() {
        ...
        $templateDefault = 'modules/shop/shop_listing';
        $templateDefaultContent = 'modules/shop/shop_content';
        ...
        if ($Website->getPosition() === 'root' || $Website->getPosition() === 'category') {
            ...
            $moduleInfo = $Website->moduleInfos($Website->getModule());
            
            $par = $moduleInfo['groupe_by'];
            ...
            $champsliste[] = $nameTable.'_traduction.short_description_tinymce';
            ...
            if (!empty($all)) {
                ...
                foreach($all as $k=>$data) {
                    ...
                    $contents[$k]['price'] = $data['price'];
                    $contents[$k]['price'] = $cart->getPriceWithPromotionWebsite($data,$id_module);//$this->Website->setCurrencyIcon((float)$contents[$k]['price']*((float)1+$cart->vat),$this->Website->configWeb['currency']);
                    $contents[$k]['stock'] = $data['quantity_stock'];
                    $contents[$k]['description'] = html_entity_decode($data['short_description_tinymce']);
                    $contents[$k]['article'] = html_entity_decode($data['article_tinymce']);
                    $contents[$k]['opt_sale'] = (bool) $data['opt_sale'];
                    $contents[$k]['opt_show_price'] = (bool) $data['opt_show_price'];
                    $contents[$k]['opt_only_web'] = (bool) $data['opt_only_web'];
                    $contents[$k]['quantity_limit'] = (bool) $data['quantity_limit'];
                    $contents[$k]['quantity_nostock'] = (bool) $data['quantity_nostock'];
                    
                    $lenArticle = strlen($contents[$k]['article']);
                    
                    if ($lenArticle > $iMaxDescription - 1) {
                        $Website->_truncate($contents[$k]['article'],$iMaxDescription);
                    }
                    
                    $contents[$k]['order'] = $data['ordre'];
                    $contents[$k]['categories'] = $data['categorie'];
                    $contents[$k]['date'] = GetDate::in($data['date_modification'],2,$Website->myLanguage);
                }
            }
            ...
            $urlAdd             = URL_USER.$Website->_lgUrl.'?controller=moduleshop&uri='.$Website->getModule().'&action=add';


            $tplModuleNewsListing = Template::getWebsiteView('modules/shop/shop_listing',$Website->getTheme());
            ...
            if (!empty($isContent)) {
                ...
                if (!empty($isContentActive)) {
                    ...
                    $isContent['description'] = html_entity_decode($isContent['short_description_tinymce']);
                    $isContent['description'] = $Website->_convertMethod($isContent['description']);
                    $isContent['title'] = $isContent['titre'];
                    $isContent['id_taxe'] = $isContentActive['id_taxe'];
                    ...
                    $isContent['price']         = $cart->getPriceWithPromotionWebsite($isContent,$id_module);  // $this->Website->setCurrencyIcon((float)$isContent['price']*((float)1+$cart->vat),$this->Website->configWeb['currency']);
                    $isContent['author_badge']  = $isContentActive['author_badge'];
                    $isContent['code']          = $isContentActive['code'];
                    ...
                    $isContent['image_gallery'] = array_merge(array($isContent['image']),$Website->_toArray($isContent['image_gallery'],';'));
                    ...
                    if (!empty($isContentActive)) {
                        ...
                        $isContent['article'] = $Website->_convertMethod($isContent['article']);
                        $isContent['description'] = html_entity_decode($isContentActiveVersion['description_tinymce']);
                        $isContent['description'] = $Website->_convertMethod($isContent['description']);
            ...
            $urlEdition         = URL_USER.$Website->_lgUrl.'?controller=moduleshop&uri='.$Website->getModule().'&action=edit&id='.$isContent['id_content'].'&lg='.$Website->getLangueTradution().'&back='.$urlAfterAction;
            $urlDelete          = URL_USER.$Website->_lgUrl.'?controller=moduleshop&uri='.$Website->getModule().'&action=delete&id='.$isContent['id_content'].'&lg='.$Website->getLangueTradution();
            $urlAdd             = URL_USER.$Website->_lgUrl.'?controller=moduleshop&uri='.$Website->getModule().'&action=add';
            
            extract($isContent);
            
            $tplModuleNewsContent = Template::getWebsiteView('modules/shop/shop_content',$Website->getTheme());
