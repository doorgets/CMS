...
<div class="doorGets-rubrique-center">
    ...
    <div class="doorGets-rubrique-center-content">
        <legend>
            [{?($is_modo):}]
            <span class="create" ><a  href="?controller=modules&action=editpartner&id=[{!$moduleInfos['id']!}]&lg=[{!$lgActuel!}]"><b class="glyphicon glyphicon-cog"></b> [{!$this->doorGets->__('Paramètres')!}]</a></span>
            [?]
            <span class="create" > <a href="?controller=module[{!$moduleInfos['type']!}]&uri=[{!$this->doorGets->Uri!}]&action=add"class="violet" ><b class="glyphicon glyphicon-plus"></b>  [{!$this->doorGets->__('Ajouter un partenaire')!}]</a></span>
            [{?($is_modo):}]
                <span class="create" >[{!$this->doorGets->genLangueMenuAdmin()!}]</span>
            [?]
            <img src="[{!BASE_IMG.'mod_partner.png'!}]" title="[{!$this->doorGets->__("Partenaire")!}]" class="doorGets-img-ico px25" />[{!$moduleInfos['nom']!}]
        </legend>
            ...
            <div style="overflow: hidden;">
                <div style="float: left;padding: 7px 0 ">
                    <i>
                        ...
                        <b>[{!$cResultsInt.' '!}] [{?( $cResultsInt > 1 ):}][{!$this->doorGets->__('Partenairess')!}] [??] [{!$this->doorGets->__('Partenaires')!}] [?]</b>
            ...
            [??]
                ...
                [{?(isset($_GET['categorie'])):}]
                    <div class="alert alert-info">
                        <i class="fa fa-exclamation-triangle"></i> [{!$this->doorGets->__("Il n'y a actuellement aucun partenaire pour cette catégorie")!}]
                    </div>
                [{???(!empty($aGroupeFilter)):}]
                    <div class="alert alert-info">
                        <i class="fa fa-exclamation-triangle"></i> [{!$this->doorGets->__("Aucun partenaire trouvé pour votre recherche");}]
                    </div>
                [??]
                    <div class="alert alert-info">
                        <i class="fa fa-exclamation-triangle"></i> [{!$this->doorGets->__("Il n'y a actuellement aucun partenaire")!}] 
                    </div>
                [?]
